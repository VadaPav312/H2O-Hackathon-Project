...
// Currently-selected US state persistence.
const DEFAULT_STATE = "CA";
const STATE_KEY = "state_abbr_v1";
...
// scripts/fetch-cdec.mjs. Older months retained from the original
// challenge_data.json snapshot for long-range chart context.
// Snowpack: % of April-1 average. Precipitation: % of average. Reservoir: %
// of capacity. Index 0 = most recent.
type WaterPoint = {
  date: string;
  snowpack: number;
  precip: number;
...
const WATER_HISTORY: WaterPoint[] = [
  { date: "4/1/26", snowpack: 11, precip: 182, reservoir: 89 },
  { date: "3/1/26", snowpack: 36, precip: 16, reservoir: 87 },
  { date: "2/1/26", snowpack: 48, precip: 155, reservoir: 83 },
  { date: "1/1/26", snowpack: 61, precip: 120, reservoir: 80 },
  { date: "12/1/25", snowpack: 27, precip: 225, reservoir: 72 },
  { date: "11/1/25", snowpack: 33, precip: 300, reservoir: 64 },
  { date: "10/1/25", snowpack: 0, precip: 108, reservoir: 62 },
  { date: "8/1/25", snowpack: 0, precip: 40, reservoir: 67 },
  { date: "7/1/25", snowpack: 0, precip: 300, reservoir: 73 },
  { date: "5/1/25", snowpack: 21, precip: 38, reservoir: 88 },
  { date: "4/1/25", snowpack: 67, precip: 19, reservoir: 89 },
  { date: "3/1/25", snowpack: 63, precip: 136, reservoir: 86 },
  { date: "2/1/25", snowpack: 61, precip: 277, reservoir: 79 },
  { date: "1/1/25", snowpack: 60, precip: 14, reservoir: 75 },
  { date: "12/1/24", snowpack: 67, precip: 275, reservoir: 72 },
  { date: "11/1/24", snowpack: 63, precip: 300, reservoir: 64 },
  { date: "10/1/24", snowpack: 0, precip: 9, reservoir: 61 },
  { date: "9/1/24", snowpack: 0, precip: 70, reservoir: 65 },
  { date: "8/1/24", snowpack: 0, precip: 300, reservoir: 69 },
  { date: "7/1/24", snowpack: 0, precip: 0, reservoir: 76 },
  { date: "6/1/24", snowpack: 40, precip: 0, reservoir: 84 },
  { date: "5/1/24", snowpack: 50, precip: 107, reservoir: 89 },
...
type AppCtx = {
  ...
  // Currently-selected US state (two-letter abbr, e.g. "CA"). Drives the
  // map, per-state data, and copy across the app. Persisted so the user's
  // home state sticks between launches.
  stateAbbr: string;
  setStateAbbr: (abbr: string) => Promise<void>;
  ...
  // Canonical single-writer for the day log: writes to AsyncStorage and
  // updates todayLog in one shot so consumers don't wait for the poll.
  appendLogEntry: (entry: CloudLogEntry) => Promise<void>;
};
type AppMode = "conservation";
...
function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [mode, setMode] = useState<AppMode>("conservation");
  const [stateAbbr, setStateAbbrState] = useState<string>(DEFAULT_STATE);
  ...
  const setStateAbbr = useCallback(async (abbr: string) => {
    if (!US_STATE_GEO[abbr]) return;
    setStateAbbrState(abbr);
    await AsyncStorage.setItem(STATE_KEY, abbr);
  }, []);
  ...
  const loadProfile = useCallback(async () => {
    ...
    if (storedState && US_STATE_GEO[storedState]) setStateAbbrState(storedState);
...
// ═══ iOS GLASS + 50-STATE WATER ATLAS ═══════════════════════════════════
// GlassCard: translucent "liquid glass" surface — hairline highlight, soft
// depth shadow, and a real backdrop blur on the web / Capacitor target.
function GlassCard({
  children,
  style,
  onPress,
  ...
  const base: any = {
    backgroundColor: (tint ?? C.surface2) + "BF",
    borderRadius: 22,
    borderWidth: 1,
    ...
    ...(Platform.OS === "web"
      ? {
          ...
          boxShadow:
            "0 10px 34px -10px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.16)",
        }
      : {
          shadowColor: "#000",
...
// US overview: all states as accurate Albers paths; selected highlighted;
// tap a data-backed state to select it.
function UsOverviewMap({
  selected,
  onSelect,
  width,
}: {
  ...
  const vb = US_VIEWBOX.split(" ").map(Number);
  const vbw = vb[2],
    vbh = vb[3];
  return (
    <Svg width={width} height={(width * vbh) / vbw} viewBox={US_VIEWBOX}>
      {Object.keys(US_STATE_GEO).map((ab) => {
        const g = US_STATE_GEO[ab];
        const isSel = ab === selected;
        const has = !!STATE_WATER[ab];
        return (
          <Path
            key={ab}
            d={g.path}
            fill={isSel ? C.accent : has ? C.surface2 : C.surface}
            fillOpacity={isSel ? 0.95 : has ? 0.92 : 0.55}
            stroke={isSel ? C.accentBright : C.border}
            strokeWidth={isSel ? 1.6 : 0.5}
            strokeLinejoin="round"
            onPress={has ? () => onSelect(ab) : undefined}
          />
        );
      })}
    </Svg>
...
// Zoomed, accurate outline of one state with its reservoirs projected on.
function StateDetailMap({ abbr, width }: { abbr: string; width: number }) {
  const g = US_STATE_GEO[abbr];
  const info = STATE_WATER[abbr];
  if (!g) return null;
  const [bx, by, bw, bh] = g.bbox;
  const pad = Math.max(bw, bh) * 0.14;
  const vw = bw + pad * 2,
    vh = bh + pad * 2;
  const vb = `${(bx - pad).toFixed(1)} ${(by - pad).toFixed(1)} ${vw.toFixed(1)} ${vh.toFixed(1)}`;
  const height = (width * vh) / vw;
  const scale = vw / width; // svg-units per screen-px
  const markers = (info?.reservoirs ?? []).map((r) => {
    const p = projectLonLat(abbr, r.lon, r.lat);
    return { ...r, x: p.x, y: p.y };
  });
  return (
    <Svg width={width} height={height} viewBox={vb}>
      <Defs>
        <SvgGradient id="stFill" x1="0" y1="0" x2="0" y2="1">
      ...
      {markers.map((m, i) => {
        const rpx = Math.max(
          4.5,
          Math.min(10, 4 + Math.sqrt((m.capacityAF ?? 120000) / 260000)),
        );
        return (
          <G key={i}>
            <Circle
...
function StatePickerModal({
  ...
  const { stateAbbr, setStateAbbr } = useApp();
  const insets = useSafeAreaInsets();
  const [q, setQ] = useState("");
  const results = STATE_LIST.filter((ab) => {
    const w = STATE_WATER[ab];
    if (!w) return false;
    const s = q.trim().toLowerCase();
    return !s || w.name.toLowerCase().includes(s) || ab.toLowerCase().includes(s);
  });
  return (
    <Modal
      visible={visible}
      ...
      onRequestClose={onClose}
    >
      <View style={{ flex: 1, backgroundColor: C.bg + "F5" }}>
        ...
        <View style={{ paddingHorizontal: 18, marginBottom: 6 }}>
          <View
            style={{
              ...
              alignItems: "center",
              gap: 8,
              backgroundColor: C.surface2 + "99",
              borderRadius: 14,
              paddingHorizontal: 12,
              borderWidth: 1,
              borderColor: C.white + "14",
            }}
          >
            <Ionicons name="search" size={16} color={C.muted} />
            <TextInput
              ...
              placeholderTextColor={C.muted}
              style={{ flex: 1, color: C.white, paddingVertical: 12, fontSize: 15 }}
            />
          </View>
        </View>
        <ScrollView
            ...
            return (
              <Press
                ...
                style={{
                  ...
                  borderRadius: 14,
                  marginBottom: 8,
                  backgroundColor: active ? C.accent + "22" : C.surface + "88",
                  borderWidth: 1,
                  borderColor: active ? C.accent + "88" : C.white + "10",
                }}
              >
                <View
                  style={{
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={{ color: C.muted, fontSize: 12, marginTop: 1 }}>
                    {w.reservoirs.length} reservoirs · {w.capital}
                  </Text>
                </View>
                {active && (
...
function StatTile({
  ...
  return (
    <GlassCard style={{ flex: 1, minWidth: 0 }}>
      ...
      >
        {value}
        {unit ? <Text style={{ fontSize: 12, color: C.muted }}> {unit}</Text> : null}
      </Text>
...
function MapScreen() {
  const { profile, stateAbbr, setStateAbbr } = useApp();
  const t = useT(profile.lang);
  const [pickerOpen, setPickerOpen] = useState(false);
  const info = STATE_WATER[stateAbbr];
  const geo = US_STATE_GEO[stateAbbr];
  const mapW = Math.min(SW - 64, 460);
  ...
  return (
    <SafeAreaView style={s.screen} edges={["top"]}>
      <GradientBg height={230} fromColor={C.accentDeep} opacity={0.22} />
      <ScreenHeader
        title="Water Atlas"
        subtitle="Reservoirs, rivers & usage · all 50 states"
      />
      <ScrollView
        showsVerticalScrollIndicator={false}
      ...
      >
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            ...
            >
              {info ? info.name : stateAbbr}
            </Text>
            <Text style={{ color: C.textSoft, fontSize: 12, marginTop: 1 }}>
              Tap to switch state
            </Text>
          </View>
          <Ionicons name="chevron-expand" size={20} color={C.white} />
        </GlassCard>

        {/* US OVERVIEW MAP */}
        <GlassCard
          style={{ marginHorizontal: 16, marginTop: 14, alignItems: "center" }}
        >
          <View
            style={{
              flexDirection: "row",
          ...
          >
            ...
            <Text style={{ color: C.muted, fontSize: 11 }}>Tap a state</Text>
          </View>
          <UsOverviewMap
            selected={stateAbbr}
        ...
        {info && geo ? (
          <>
            ...
            >
              <View style={{ alignSelf: "stretch", marginBottom: 8 }}>
                <Text
                  style={{ color: C.white, fontSize: 18, fontWeight: "900" }}
                >
                  {info.name}
                </Text>
                <Text style={{ color: C.muted, fontSize: 12 }}>
                  {info.nickname} · {info.reservoirs.length} major reservoirs
                </Text>
              </View>
              <StateDetailMap abbr={stateAbbr} width={mapW} />
              <View
                style={{
                  flexDirection: "row",
                  gap: 14,
            ...
            >
              <StatTile
                icon="rainy"
                label="Avg precip"
                value={String(Math.round(info.avgAnnualPrecipIn))}
                unit="in/yr"
              />
              <StatTile
                icon="person"
                label="Per person"
                value={String(Math.round(info.perCapitaGalDay))}
                unit="gal/d"
              />
            </View>
            <View
            ...
            >
              <StatTile
                icon="water"
                label="Reservoirs"
                value={String(info.reservoirs.length)}
              />
              <StatTile
                icon="people"
                label="Population"
                value={fmtBig(info.population)}
              />
            </View>
            ...
            <GlassCard style={{ marginHorizontal: 16, marginTop: 14 }}>
              ...
              >
                ...
                />
                <Text
                  style={{ flex: 1, color: C.muted, fontSize: 12, lineHeight: 18 }}
                >
                  {info.droughtNote}
                </Text>
              </View>
              {info.agencyUrl ? (
                <Press
                  onPress={() => Linking.openURL(info.agencyUrl!)}
                  style={{ marginTop: 12, flexDirection: "row", gap: 6 }}
                >
            ...
            <GlassCard style={{ marginHorizontal: 16, marginTop: 14 }}>
              ...
              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
                {info.majorRivers.map((r) => (
                  ...
                  >
                    <Text
                      style={{ color: C.accentBright, fontSize: 12, fontWeight: "700" }}
                    >
                      {r}
                    </Text>
                  </View>
                ...
                >
                  <View
                    style={{
                      ...
                      height: 12,
                      borderRadius: 6,
                      backgroundColor: resColor(r.pctAvg),
                    }}
                  />
                  <View style={{ flex: 1 }}>
                    <Text
                    ...
                    <Text style={{ color: C.muted, fontSize: 11, marginTop: 1 }}>
                      {r.river ? r.river + " · " : ""}
                      {r.capacityAF ? fmtBig(r.capacityAF) + " ac-ft" : ""}
                    </Text>
                  </View>
                  {r.pctAvg != null && (
                    <Text
                      style={{
                        color: resColor(r.pctAvg),
                        fontSize: 14,
                        fontWeight: "900",
                      }}
                    >
            ...
            <GlassCard style={{ marginHorizontal: 16, marginTop: 14 }}>
              ...
              {info.facts.map((f, i) => (
                ...
                >
                  ...
                  <Text
                    style={{ flex: 1, color: C.textSoft, fontSize: 13, lineHeight: 19 }}
                  >
                    {f}
                  </Text>
                </View>
          ...
          <GlassCard style={{ margin: 16 }}>
            <Text style={{ color: C.muted, fontSize: 13 }}>
              No detailed data for this state yet.
            </Text>
          </GlassCard>
        )}
      </ScrollView>
      <StatePickerModal visible={pickerOpen} onClose={() => setPickerOpen(false)} />
    </SafeAreaView>
  );
}

function LegacyCaliforniaMapScreen() {
  const { profile } = useApp();
  const t = useT(profile.lang);
  ...
  heroCard: {
    ...
    ...(Platform.OS === "web"
      ? ({
          ...
          boxShadow:
            "0 14px 40px -12px rgba(0,0,0,0.55), inset 0 1px 0 rgba(255,255,255,0.18)",
        } as any)
      : SHADOW_HERO),
  },
  heroLabel: {
  ...
  glassCard: {
    ...
    ...(Platform.OS === "web"
      ? ({
          ...
          boxShadow:
            "0 8px 30px -10px rgba(0,0,0,0.45), inset 0 1px 0 rgba(255,255,255,0.14)",
        } as any)
      : SHADOW),
  },
